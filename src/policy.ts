import { resolveRoute, substitute, type Route, type RouterConfig } from "./config.ts";
import { known, maxUsed, type Vendor, type VendorUsage } from "./usage.ts";
...
export function selectRoute(
  config: RouterConfig,
  role: string,
  usage: { claude: VendorUsage; codex: VendorUsage },
): Selection {
  ...
  const otherVendor: Vendor = route.vendor === "claude" ? "codex" : "claude";
