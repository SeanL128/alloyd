...
export type Tier = { vendor: Vendor; model: string; effort: string };
export type Band = { name: string; models: Partial<Record<Vendor, string>> };
export type RouterConfig = {
  roles: Record<string, Tier>;
  bands: Band[];
  dispatch: Record<Vendor, string>;
};
export type Route = { role: string; vendor: Vendor; model: string; effort: string };
...
export function resolveRoute(config: RouterConfig, role: string): Route {
...
export function substitute(config: RouterConfig, route: Route, toVendor: Vendor): Route {
  if (route.vendor === toVendor) return route;
...
export function buildDispatch(config: RouterConfig, route: Route, prompt: string): string {
...
export function parseConfig(json: string): RouterConfig {
  const cfg = JSON.parse(json) as RouterConfig;
  if (!cfg || typeof cfg !== "object") throw new Error("config must be an object");
  if (!cfg.roles || typeof cfg.roles !== "object") throw new Error("config.roles is missing");
  if (!Array.isArray(cfg.bands)) throw new Error("config.bands must be an array");
  if (!cfg.dispatch || typeof cfg.dispatch !== "object") throw new Error("config.dispatch is missing");
  ...
  for (const [role, tier] of Object.entries(cfg.roles)) {
    if (!tier || typeof tier !== "object") throw new Error(`role ${role} must be an object`);
  ...
  for (let i = 0; i < cfg.bands.length; i++) {
    ...
    if (!band || typeof band !== "object") throw new Error(`band ${i} must be an object`);
