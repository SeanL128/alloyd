...
import { executeDispatch, formatRouteSummary, loadConfig, prepareDispatch, readUsage } from "./pipeline.js";
import { suggestRole } from "./suggest.js";
const server = new McpServer({ name: "alloyd", version: "0.0.0" });
const jobs = new Map();
let nextJob = 1;
function formatDispatchResult(result, dryRun = false) {
...
server.registerTool("dispatch", {
    ...
    inputSchema: z.object({
        ...
        brief: z.object({
            ...
            files: z.array(z.string()).describe("The minimal set of files the task touches. The narrower this list, the faster the subagent grounds — name exact paths rather than leaving it broad. Empty only when the files genuinely cannot be known up front."),
    ...
    if ("ok" in prepared)
        return formatDispatchResult(prepared);
    if (dry_run)
        return formatDispatchResult({ ok: true, ...prepared, exitCode: 0 }, true);
    if (background) {
        const jobId = `job-${nextJob++}`;
        const job = { promise: executeDispatch(prepared), startedAt: Date.now(), role };
        jobs.set(jobId, job);
...
server.registerTool("dispatch_result", {
    ...
    if (!job)
        return { isError: true, content: [{ type: "text", text: `unknown job: ${job_id}` }] };
    if (!job.result) {
