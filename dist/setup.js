...
export function updateSettingsContent(content, scriptPath) {
    let config;
    try {
...
export function updateInstructionsContent(content, snippet) {
    const block = `<!-- alloyd:start -->\n${snippet}<!-- alloyd:end -->\n`;
    const sentinelPattern = /<!-- alloyd:start -->[\s\S]*?<!-- alloyd:end -->\r?\n?/;
    if (sentinelPattern.test(content))
        return content.replace(sentinelPattern, block);
    if (!content)
        return block;
    if (content.endsWith("\n\n"))
...
export function detectMode(content) {
    ...
    if (!match)
        return null;
    if (match[1].includes("alloyd dispatch"))
        return "cli";
    if (match[1].includes("MCP tool"))
        return "mcp";
    return null;
}
function snippetPathFor(pkgRoot, mode) {
    return join(pkgRoot, "plugin-assets", mode === "cli" ? "CLAUDE-snippet-cli.md" : "CLAUDE-snippet.md");
}
function applySnippet(filePath, snippet, dryRun) {
...
export function runSetup(dryRun, modeArg) {
    const pkgRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");
    ...
    if (updated === null) {
        ...
        if (updated.wrappedCommand)
            console.log(`  (wraps your existing statusline, whose output is kept: ${updated.wrappedCommand})`);
    }
    else {
    ...
    console.log(mdResult === "unchanged"
    ...
    console.log("\n== Codex side ==");
    const codex = spawnSync(process.execPath, [join(pkgRoot, "scripts", "setup-codex.mjs"), "--snippet", snippetPath, ...(dryRun ? ["--dry-run"] : [])], { stdio: "inherit" });
    console.log(`
============================================================
